#!/usr/bin/env bash
# aiutilx installer — Linux and macOS
...
if [ "${BUILD_FROM_SOURCE:-0}" != "1" ]; then
  ...
  if [ -n "$LATEST" ]; then
    URL="https://github.com/${REPO}/releases/download/${LATEST}/aiutilx-${PLATFORM}.tar.gz"
    TMP=$(mktemp -d)
    trap 'rm -rf "$TMP"' EXIT
...
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT
git clone --depth=1 "https://github.com/${REPO}.git" "$TMP/aiutilx"
cd "$TMP/aiutilx"
